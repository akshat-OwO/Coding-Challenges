...
peeps.forEach((p) => (p.favoriteNumbersSum = p.favoriteNumbers.reduce((a, b) => a + b)));
...
const numberOnePeeps = peeps.filter((p) => p.favoriteNumbers.includes(1)).map((item) => {
  return {
    id: item.id,
    name: item.name
  }
...
youngPeeps.forEach((p) => {
  if (numberOnePeeps.find((fp) => p.id === fp.id)) {
    freshPeeps.push(p);
  }
});
